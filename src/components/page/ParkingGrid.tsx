      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {levels.map((level, index) => (
          <div key={index} className="flex flex-col items-center space-y-4">
            <h3 className="text-lg font-medium text-gray-700">Level {index + 1}</h3>
            ...
            <div className="grid grid-cols-3 gap-2">
              {level.print().map((spot, spotIndex) => {
                ...
                return (
                  <div
                    key={spotIndex}
                    className={`h-16 w-16 bg-gray-200 rounded-lg flex items-center justify-center text-gray-700 text-sm font-semibold`}
                  >
                    {spot}
                  </div>
                );
